function KhongDau(str, rules){
	/*
		rules: [rule_1, rule_2,...]
			...
			+ "chuan" (Chuẩn hóa chuỗi - Bỏ các khoảng trắng, cách dấu xuống dòng \n \r)
			+ "thuong" (Chuyển các kí tự thành kí tự thường)
			+ "hoa" (Chuyển các kí tự thành kí tự hoa)
			+ "url" (Chuẩn hóa chuỗi để sử dụng cho các liên kết)
			+ "file" (Chuẩn hóa chuỗi để sử dụng làm tên file)
			+ "ascii" (Chuyển Tiếng Việt sang các kí tự trong bảng mã ASCII)
